...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const displayImage = product.image || (product as any).thumbnailUrl || 'https://placehold.co/600x400/001f3f/white?text=No+Image';
  const displayAuthor = product.author || (product as any).teacherName || 'TestKart Expert';
  ...
  return (
    ...
    >
      ...
      <View className="absolute top-3 left-3 z-10 px-3 py-1.5 bg-white/90 dark:bg-slate-900/90 rounded-full border border-gray-100 dark:border-slate-700/50 shadow-sm">
        ...
        <View className="p-5">
          <Text className="text-xl font-bold text-slate-800 dark:text-white mb-2 leading-tight" numberOfLines={2}>
            {product.title}
          </Text>
          ...
          <View className="flex-row items-center mb-4">
            <View className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 mr-2 items-center justify-center overflow-hidden">
                <Image source={{ uri: (product as any).teacherAvatar || `https://ui-avatars.com/api/?name=${displayAuthor}&background=random` }} className="w-full h-full" />
            </View>
          ...
          <View className="flex-row items-center justify-between pt-4 border-t border-gray-50 dark:border-slate-700">
            <View className="flex-row items-center">
              <Feather name="shopping-bag" size={14} color={colorScheme === 'dark' ? '#94a3b8' : '#64748b'} />
