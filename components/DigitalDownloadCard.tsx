    ...
    >
      <View className="p-4">
        ...
        <View className="px-1 pb-2">
          <Text className="text-xl font-black text-slate-800 dark:text-white mb-4 leading-tight" numberOfLines={2}>
            {product.title}
          </Text>
          ...
          <View className="flex-row items-center mb-6">
            <View className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 mr-3 items-center justify-center overflow-hidden border-2 border-white dark:border-slate-800 shadow-sm">
                <Image 
                  source={{ uri: product.teacherAvatar || `https://ui-avatars.com/api/?name=${displayAuthor}&background=random` }} 
                  className="w-full h-full" 
                />
            </View>
          ...
          <View className="flex-row items-center justify-between pt-5 border-t border-slate-50 dark:border-slate-700/50">
            <View className="flex-row items-center">
              <View className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded-xl mr-2.5">
                <Feather name="shopping-bag" size={16} color={colorScheme === 'dark' ? '#94a3b8' : '#64748b'} />
              </View>
            ...
            {displayPrice === 0 ? (
              ...
              <Text className="text-3xl font-black text-primary">₹ {displayPrice}</Text>
            )}
          </View>
        </View>
      </View>
