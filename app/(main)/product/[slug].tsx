...
import { View, Text, ScrollView, TouchableOpacity, Image, ActivityIndicator, StatusBar, Share } from 'react-native';
...
const ProductDetails = () => {
    ...
    const fetchProductDetails = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${BASE_URL}/_api/shop/details?slug=${slug}`);
            ...
            setRelatedProducts(listPayload.products?.filter((p: any) => p.slug !== slug) || []);
        } catch (error: any) {
    ...
    if (!product) {
        return (
            <View className="flex-1 bg-white dark:bg-slate-900 items-center justify-center px-10">
                <Text className="text-xl font-bold text-slate-800 dark:text-white mb-4">Product not found</Text>
                <TouchableOpacity
                    onPress={() => router.back()}
                ...
                >
                    ...
                    <View className="px-5 pt-4">
                        <View className="relative w-full h-[260px] rounded-[32px] overflow-hidden bg-slate-100 dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-800">
                            <Image
                                source={{ uri: product.thumbnailUrl || product.image || 'https://placehold.co/600x400/001f3f/white?text=TestKart' }}
                    ...
                    <View className="px-6 py-6">
                        ...
                        <View className="flex-row items-center justify-between mb-8">
                            <View className="flex-row items-center flex-1">
                                <View className="w-12 h-12 rounded-full border-2 border-white dark:border-slate-800 shadow-sm overflow-hidden mr-3">
                                    <Image 
                                        source={{ uri: product.teacherAvatar || `https://ui-avatars.com/api/?name=${product.teacherName || product.author}&background=FF8A50&color=fff` }} 
                                        className="w-full h-full" 
                                    />
                                </View>
                                <View>
                                    <Text className="text-slate-500 dark:text-slate-400 text-sm font-bold">By</Text>
                                    <Text className="text-slate-800 dark:text-white font-black text-lg leading-tight -mt-0.5">
                                        {product.teacherName || product.author || 'TestKart Expert'}
                                    </Text>
                                </View>
                            </View>
                            ...
                            <View className="flex-row items-center bg-gray-50 dark:bg-slate-800/50 px-4 py-2.5 rounded-2xl border border-gray-100 dark:border-slate-700">
                                ...
                                <Text className="mx-1.5 text-slate-800 dark:text-white font-black">{product.rating ? product.rating.toFixed(1) : 'New'}</Text>
                                <Text className="text-slate-500 dark:text-slate-400 text-xs font-bold leading-none">({product.reviewsCount || 0} reviews)</Text>
                            </View>
                        </View>
                        ...
                        <View className="mb-10">
                            ...
                            <Text className="text-slate-600 dark:text-slate-300 text-base leading-7">
                                {product.shortDescription || product.description?.replace(/<[^>]*>?/gm, '') || "No description available for this product."}
                            </Text>
                        </View>
                        ...
                        {product.tags && product.tags.length > 0 && (
                            <View className="mb-10">
                                <Text className="text-2xl font-black text-slate-800 dark:text-white mb-4">Tags</Text>
                                <View className="flex-row flex-wrap">
                                    {product.tags.map((tag: string, i: number) => (
                                        <View key={i} className="bg-cyan-50 dark:bg-cyan-900/20 px-5 py-2.5 rounded-2xl border border-cyan-100 dark:border-cyan-800/30 mr-3 mb-3">
                                            <Text className="text-cyan-600 dark:text-cyan-400 font-black text-base uppercase tracking-tight">{tag}</Text>
                                        </View>
                                    ))}
                                </View>
                            </View>
                        ...
                        <View className="mb-10">
                            <Text className="text-2xl font-black text-slate-800 dark:text-white mb-6">About the Author</Text>
                            <View className="bg-white dark:bg-slate-800/80 p-6 rounded-[32px] border border-gray-100 dark:border-slate-700 shadow-sm">
                                <View className="flex-row items-center">
                                    <View className="w-20 h-20 rounded-3xl bg-slate-100 dark:bg-slate-700 overflow-hidden shadow-sm">
                                        <Image source={{ uri: product.teacherAvatar || `https://ui-avatars.com/api/?name=${product.teacherName}&size=200&background=FF8A50&color=fff` }} className="w-full h-full" />
                                    </View>
                                    <View className="ml-5 flex-1">
                                        <Text className="text-2xl font-black text-slate-800 dark:text-white mb-2">{product.teacherName || 'Author'}</Text>
                                        <TouchableOpacity className="bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800/50 px-5 py-2 rounded-xl self-start">
                                            <Text className="text-primary font-black text-sm">View Profile</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </View>
                        ...
                        <View className="bg-white dark:bg-slate-800/50 p-5 rounded-[28px] border border-gray-100 dark:border-slate-700/80 mb-10 shadow-lg shadow-slate-200/50 dark:shadow-none">
                            <View className="flex-row items-center justify-between mb-4">
                                <Text className="text-slate-500 dark:text-slate-400 text-base font-bold uppercase tracking-wider">Price</Text>
                                <Text className="text-3xl font-black text-orange-500">
                                    {product.price === 0 ? <Text className="text-green-500">FREE</Text> : `₹ ${product.price}`}
                                </Text>
                            </View>
                            ...
                            <TouchableOpacity className="bg-primary h-14 rounded-2xl flex-row items-center justify-center shadow-md shadow-orange-500/20 mb-3">
                                ...
                                <Text className="text-white text-lg font-black ml-2.5">Add to Cart</Text>
                            </TouchableOpacity>
                            ...
                            <View className="flex-row items-center justify-between mb-6 px-1">
                                <TouchableOpacity className="flex-1 h-12 rounded-2xl bg-orange-50/60 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-800/40 items-center justify-center mr-2 shadow-sm shadow-orange-100/20">
                                    <View className="flex-row items-center">
                                        ...
                                        <Text className="text-primary font-black text-sm ml-2">Preview</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    onPress={handleShare}
                                ...
                                >
                                    <View className="flex-row items-center">
                                        ...
                                        <Text className="text-primary font-black text-sm ml-2">Share</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                            ...
                            <View className="px-1 space-y-3.5">
                                <View className="flex-row items-center">
                                    ...
                                    <Text className="text-slate-700 dark:text-slate-300 font-bold text-base">{product.pageCount || 0} Pages</Text>
                                </View>
                                <View className="flex-row items-center">
                                    <View className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800/80 items-center justify-center mr-3">
                                        <Feather name="download" size={16} color="#64748b" />
                                    </View>
                                    <Text className="text-slate-700 dark:text-slate-300 font-bold text-base">{formatFileSize(product.fileSizeBytes)} PDF</Text>
                                </View>
                                <View className="flex-row items-center">
                                    <View className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800/80 items-center justify-center mr-3">
                                        <Feather name="globe" size={16} color="#64748b" />
                                    </View>
                                    <Text className="text-slate-700 dark:text-slate-300 font-bold text-base">{product.language || 'English Language'}</Text>
                                </View>
                                <View className="flex-row items-center">
                                    <View className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800/80 items-center justify-center mr-3">
                                        <Feather name="users" size={16} color="#64748b" />
                                    </View>
                                    <Text className="text-slate-700 dark:text-slate-300 font-bold text-base">{product.totalPurchases || 0} Downloads</Text>
                                </View>
                            </View>
                        ...
                        {relatedProducts.length > 0 && (
                            <View className="mt-4">
                                ...
                                <Text className="text-3xl font-black text-slate-800 dark:text-white mb-8">Related Products</Text>
