...
const EnrolledCourseCard = ({ course }: EnrolledCourseCardProps) => {
  return (
    <TouchableOpacity 
      onPress={() => router.push(`/(main)/course/${course.slug}` as any)}
    ...
    >
      {/* Small Thumbnail with Play Indicator */}
      <View className="w-20 h-20 rounded-2xl bg-slate-100 dark:bg-slate-800 mr-4 overflow-hidden relative">
        <Image 
          source={{ uri: course.thumbnailUrl || "https://ik.imagekit.io/testkart/placeholders/mock-test-placeholder__FmYrad7s.png" }} 
      ...
      <View className="flex-1">
        <Text className="text-slate-800 dark:text-white font-black text-lg mb-0.5" numberOfLines={1}>
          {course.title}
        </Text>
